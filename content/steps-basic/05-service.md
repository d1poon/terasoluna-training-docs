...
認証が絡まないため、Service のメソッドは Repository の CRUD 4 種にほぼ 1:1 対応する素直な構成になる。

## 事前準備

- [Step 04](/steps-basic/04-repository) 完了 (Repository が動き、テーブルが作られている)
...
- **② interface で公開 API を定義** — Controller や別 Service からは interface 経由でしか呼べない。メソッドは Step 04 の Repository CRUD 4 種 (`findAll` / `findById` / `findByRole` / `update`) にそのまま対応する
...
- **⑥ `@Transactional(readOnly = true)`** — 参照系メソッド (`findAll` / `findById` / `searchByRole`) の最適化ヒント。書き込みしないことを DB に伝える。`update` だけは書き込みなので付けない (クラス全体の `@Transactional` がそのまま効く)
...
→ [Step 06: ユーザ一覧画面 (List Controller + JSP)](/steps-basic/06-list)
