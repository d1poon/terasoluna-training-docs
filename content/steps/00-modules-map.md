...
- **時間の目安は載せていません**。速く進めることより、**各行の意味を自分の言葉で説明できる状態**を優先してください。1 つの Step を 何度読み返しても、日をまたいで再開しても大丈夫な構成にしています
- **順番通りに一つずつ**進めてください。前の Step の成果物 (ファイル / 設定) を次で使うので、飛ばすと詰まります
- 各 Step は決まった構造で書かれています:
  1. **このステップのゴール** — 何が出来るようになるか
  2. **追加/修正するファイル** — 「どこに何のコードを置くか」の階層図付き
  3. **なぜこう書く** — 各コード行の意図。**動くコードを書いた後で読む**のが理解しやすい
  4. **動作確認** — ここまでで何が動く状態になったかチェック
  5. **よくある詰まり** — 遭遇しやすいエラーと対処
- 動かない時は、次の順で当たってください:
  - 該当 Step 末尾の「よくある詰まり」
  - [トラブルシュート](/troubleshoot) — サイト全体の詰まりを 10 項目に集約
  - [用語集](/glossary) — 分からない単語をここで引く
